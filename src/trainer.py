...
class PINNTrainer:
    ...
    def train_adam(self, epochs: int=500, lr: float=1e-3, log_every: int=500):
        ...
        print(f"{'='*55}")
        print(f" Adam | epochs={epochs} | lr={lr}")  # Fix 4: added = after lr
        print(f"{'='*55}")
        ...
        for epoch in pbar:
            ...
            # Fix 1: history now appended every epoch unconditionally
    ...
    def train_lbfgs(self, max_iter: int=500, lr: float=1.00, log_every: int=100):
        ...
        print(f"{'='*55}")
        print(f" L-BFGS | max_iter={max_iter} | lr={lr}")  # Fix 2: epochs -> max_iter
        print(f"{'='*55}")
        ...
        def closure():
            ...
            if step[0] % log_every == 0:
                print(f" iter {step[0]:04d} | total={l_total.item():.4e} | ode={l_ode.item():.4e} | bc={l_bc.item():.4e}")  # Fix 3: :.4d -> :04d
            step[0] += 1
            return l_total
